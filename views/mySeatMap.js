define(['jquery','underscore','backbone','SGView','text!templates/mySeatMap.html','SeatCharts'],
function($,_,Backbone,SGView,viewtTpl){
	var MySeatMap = SGView.extend({
		...
		render:function(){
			...
			self.$el.append(self.template({lang:self.lang,data:self.recommendedSeat,pnrTraList:pnrTraList,RecommendSeat:RecommendSeat,name:self.params.nowMyFlightInfo.username,operating:self.params.operating,priceList:priceList}));
...
//				$("#wrapper").width(document.body.clientWidth);
		...
		PrimarySeatSelect:function(){
			var self = this;
			var pnrNo = self.params.guideRsp.pnr;
			
			var cabin = self.params.guideRsp.mainCabin;;
			var phoneNO = "";
			var airCd = self.params.guideRsp.airCode;
			var fltNumber = self.params.guideRsp.fltNumber;
			var flightNo = airCd+fltNumber;
			var depAirportCode = self.params.guideRsp.deCode;
			var arrAirportCode = self.params.guideRsp.arCode;
			var arrDate = self.params.guideRsp.adate;
			var arrTime = self.params.guideRsp.atime;
			var depDate = self.params.guideRsp.deDate;
			var depTime = self.params.guideRsp.deTime;
			var flightDate = depDate+" "+depTime;
			var carrierFlightNo = self.params.guideRsp.opAirFltNumber;
			var couponNum = self.params.guideRsp.couponNumber;
			...
			self.sgClient.post(query,'',function(_data){			
				if(_data && _data.code == 0){//查询成功
					...
					self.jumpTo("forward/myprimaryseatdetails");
				}else{//失败
		...
		doCheckin:function(){
			...
			var flightDate = self.params.guideRsp.deDate;
			var airCd = self.params.guideRsp.airCode;
			var fltNumber = self.params.guideRsp.fltNumber;
			var flightNo = airCd+fltNumber;
			var fromCity = self.params.guideRsp.deCode;
			var toCity = self.params.guideRsp.arCode;
			var tourIndex = self.params.guideRsp.couponNumber;
			var flightClass = self.params.guideRsp.mainCabin;
			var phoneNo = "";
			var arrDate = self.params.guideRsp.adate;
			var depTime = self.params.guideRsp.deTime;
			var arrTime = self.params.guideRsp.atime;
			var actualFlyDate = self.params.guideRsp.deDate;
			var carrierFlightNo = self.params.guideRsp.opAirFltNumber;
			var userInfoList = $('#headList').children('li');
			var passengerList = [];
			passengerList[0] = {
			...
			self.sgClient.post(query,'',function(_data){			
				if(_data && _data.code == 0){//查询成功
					...
					self.jumpTo("forward/myseatselectiondetails");
				}else{//失败
					self.utils.showAlert({text:(_data.msg)});
				}
				self.utils.hideMessage();
