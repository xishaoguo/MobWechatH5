...
//				}
			...
			return function(i) {
				...
				s.settings.$node
						...
						.addClass(
								[ "seatCharts-seat", "seatCharts-cell",
										...
										.concat(
												s.settings.classes,
												typeof n.seats[s.settings.character] == "undefined" ? []
				...
				s.style = function() {
					return arguments.length == 1 ? function(e) {
						...
						s.settings.status = e != "focused" ? e
								: s.settings.status;
						s.settings.$node.attr("aria-checked", e == "selected");
						n.animate ? s.settings.$node.switchClass(t, e, 200)
								: s.settings.$node.removeClass(t).addClass(e);
						return s.settings.style = e
					}(arguments[0]) : s.settings.style
				};
				s.status = function() {
					return s.settings.status = arguments.length == 1 ? s
							.style(arguments[0]) : s.settings.status
				};
				(function(i, o, u) {
					e
							.each(
									...
									function(e, a) {
										s[a] = function() {
											if (a == "focus") {
												if (t
														.attr("aria-activedescendant") !== undefined) {
													r[t
											...
											return s
													.style(typeof i[o][a] === "function" ? i[o][a]
															.apply(u)
															: n[a].apply(u))
										}
									})
								...
								function(e, n) {
									return function(i) {
										...
										case 38:
													...
													(
															...
															n
																	...
																	.find(
																			".seatCharts-seat,.seatCharts-space"),
															n
																	.parents(".seatCharts-row:not(.seatCharts-header)"));
											if (!s.length) {
												return
											}
											e.blur();
											r[s.attr("id")].focus();
											s.focus();
											t.attr("aria-activedescendant", s
													.attr("id"));
											break;
										case 37:
										case 39:
				...
				.each(
						...
						function(t, s) {
							...
							e
									.each(
											s
													.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi),
											function(n, s) {
												var f = s
														.match(/([a-z_]{1})(\[([0-9a-z_ ,]+)\])?/i), l = f[1], c = typeof f[3] !== "undefined" ? f[3]
														.split(",")
														: [], h = c.length ? c[0]
														: null, p = c.length === 2 ? c[1]
														: null;
												a
														.append(l != "_" ? function(
																e) {
		...
		o.legend.items.length ? function(t) {
			var r = (t.node || e("<div></div").insertAfter(n))
					.addClass("seatCharts-legend");
			var i = e("<ul></ul>").addClass("seatCharts-legendList")
					.appendTo(r);
			e
					.each(
							t.items,
		...
		n
				.data(
						"seatCharts",
						{
							...
							status : function() {
								var t = this;
								return arguments.length == 1 ? t.seats[arguments[0]]
										.status()
										: function(n, r) {
											return typeof n == "string" ? t.seats[n]
													.status(r)
													: function() {
														e
																.each(
								...
								return {
									...
									status : function() {
										...
										return this.length == 1
												&& t.length == 0 ? this.seats[0]
												.status()
												: function() {
													e.each(n.seats, function() {
														this.status.apply(this,
																t)
													})
												}()
									},
							...
							get : function(t) {
								var n = this;
								return typeof t == "string" ? n.seats[t]
										: function() {
											var r = n.set();
											e
													.each(
